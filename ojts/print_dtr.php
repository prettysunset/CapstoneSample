...
$logged_in_role = strtolower(trim((string)($_SESSION['role'] ?? '')));
if ($logged_in_role !== '' && $logged_in_role !== 'ojt') {
...
// For OJT self-service printing, always use the logged-in user's own student record.
...
if (!$student) {
    echo "<p>Student record not found for this account.</p>";
    exit;
}
