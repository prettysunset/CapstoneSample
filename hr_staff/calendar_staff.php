...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER['CONTENT_TYPE'] ?? '', 'application/json') !== false) {
  ...
  error_log('calendar_staff.php POST payload: ' . $raw);
      ...
      if ($_SERVER['REQUEST_METHOD'] === 'POST' && strpos($_SERVER['CONTENT_TYPE'] ?? '', 'application/json') !== false) {
        ...
        if ($input && ($input['action'] ?? '') === 'reschedule_all') {
          ...
          ob_end_clean(); header('Content-Type: application/json; charset=utf-8'); echo json_encode(['success'=>true,'message'=>'Rescheduled','target_session_id'=>$targetSessionId,'from_date'=>$fromLabel,'deleted_original'=>!empty($deletedOriginal)]); exit;
            ...
            $buf = ob_get_clean();
            error_log('calendar_staff.php reschedule exception: ' . $e->getMessage() . '\nBuffer:\n' . $buf . '\nTrace:\n' . $e->getTraceAsString());
  ...
  <script>
    ...
    document.addEventListener('click', function(e){
      ...
      if (p) {
        ...
        var url = 'print_endorsement_staff.php?';
      ...
      try {
        ...
        function fetchReschedCountsForRange(startIso, endIso){
          try {
            ...
            return fetch('calendar_staff.php', {
    ...
    document.getElementById('resched_confirm').addEventListener('click', function(){
      ...
      fetch('calendar_staff.php', {
        ...
        return r.text().then(function(txt){
          ...
          catch(e) { console.warn('calendar_staff.php non-JSON response:', txt); return { success: r.ok, message: txt }; }
        });
      }).then(function(res){
